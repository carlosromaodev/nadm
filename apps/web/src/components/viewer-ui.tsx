...
export function ProfileCard({ profile }: { profile: ViewerProfile }) {
  ...
  return <Link href={`/${profile.handle}`} className="flex items-center gap-3 rounded-[24px] border border-line bg-surface p-[13px] hover:bg-surface2">
    ...
    <span className="min-w-0 flex-1"><span className="flex items-center gap-1.5 truncate text-[14px] font-[900]">{profile.displayName}{profile.verified && <Flor className="size-3.5 shrink-0 text-lime-text" />}</span><span className="mt-0.5 block truncate text-[11.5px] font-[700] text-dim">{[profile.category, profile.location].filter(Boolean).join(' · ') || `@${profile.handle}`}</span><span className="mt-1.5 block text-[10.5px] font-[800] text-dim">{profile.responseTimeHours ? `Responde em ~${profile.responseTimeHours}h · ${profile.completedDeals ?? 0} trabalhos` : profile.availabilityStatus === 'AVAILABLE' ? 'A aceitar pedidos' : profile.availabilityStatus === 'PAUSED' ? 'Em pausa' : 'Sem vagas'}</span></span>
